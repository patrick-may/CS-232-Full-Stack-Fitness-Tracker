...
def test_insert_set(flask_test_client):
    ...
    WHEN a set is inserted
    THEN check if set is correctly in the database and API return is correct
    """
    json_set = {"gym_id": 1, "machine_name": "Squat Rack 2",
                "exercise_date": "05/12/2022", "reps": 5, "weight": 225}
...
def test_update_set(flask_test_client):
    ...
    WHEN a set is updated
    THEN check if set is correctly updated in the database and API return is correct
    """
    # update sets and reps
    json_set_update = {"gym_id": 1, "machine_name": "Squat Rack 2",
                       "exercise_date": "05/12/2022", "reps": 10, "weight": 15}

    request = flask_test_client.put(
        "/api/v1/weight_sets/", json=json_set_update)
...
def test_delete_set(flask_test_client):
    ...
    THEN check if set is in the database and API return is correct
    """
    json_delete = {"gym_id": 1, "machine_name": "Squat Rack 2",
                   "exercise_date": "05/12/2022"}

    request = flask_test_client.delete(
        "/api/v1/weight_sets/", json=json_delete)
...
def test_views(flask_test_client):
    ...
    WHEN a set is in the database
    THEN check if set is returned correctly from the database and API return is correct
    """
    new_set_1 = {"gym_id": 1, "machine_name": "Pull Up Bar",
                 "exercise_date": "05/12/2022", "reps": 7, "weight": 25}
    new_set_2 = {"gym_id": 3, "machine_name": "Squat Rack 2",
                 "exercise_date": "05/13/2022", "reps": 5, "weight": 225}
